...
| `TemplateStatusHandler` | `webhooks/handlers/template-status.handler.spec.ts` | ✅ APPROVED / REJECTED / PAUSED / PENDING_DELETION / unknown / `"NONE"` sentinel |
...
| Where the rejection reason actually is | Info | `message_template_status_update` carries the enum in `reason` and the sentence worth reading in `other_info` (`{ title, description }`, sometimes as a JSON string). Reading `reason` alone threw the sentence away, and stored nothing at all when Meta sent `reason: "NONE"` beside a populated `other_info` — the console then showed a rejected template with no explanation. `templateStatusReason()` takes the most specific of the three: description, then title, then the enum |
